...
__global__ void createTemperatureTable(const int size, double* table, const float min_disk_r, const float step_size, float M, float Ma);
__global__ void addAccretionDisk(const float2* thphi,const float3* disk_incident, uchar4* out, double* temperature_table, const float temperature_table_step_size, const int temperature_table_size,
	const unsigned char* bh, const int M, const int N, const float* camParam, const float* solidangle, const float* solidangle_disk, float2* viewthing, float min_disk_r, float max_disk_r, bool lensingOn, const unsigned char* diskMask);
